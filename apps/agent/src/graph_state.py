...
class GraphMemoryState(MessagesState):
    ...
    real_prices: list[dict] = Field(
        default_factory=list, description="Precios reales de la BD"
    )
    shopping_list: str = Field("Shopping list")
    corrections: str | None = Field("Corrections of the shopping list if exists")
